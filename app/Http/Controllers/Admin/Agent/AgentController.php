...
class AgentController extends Controller
{
    ...
    public function getAgent(Request $request)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($data) {
                ...
                $button = '<a   id="' . $data->id . '" name_ar="' . $data->getTranslation('name', 'ar') . '" name_en="' . $data->getTranslation('name', 'en') . '"  location_ar="' . $data->getTranslation('location', 'ar') . '"   location_en="' . $data->getTranslation('location', 'en') . '" mobile="' . $data->mobile . '" class="edit_agent"><span><i style="color: #66afe9" class="fas fa-edit"></i></span></a>';
                $button .= '&nbsp;&nbsp;&nbsp;&nbsp;';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $agent = new Agent();
            $agent->name = ['ar' => $request->name_ar, 'en' => $request->name_en];
            $agent->location = ['ar' => $request->location_ar, 'en' => $request->location_en];
